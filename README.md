...
This project supports claims about acoustic structure, site-controlled label decodability (ecotype and catalogue call type), cross-site call-type transfer, non-random clustering, non-random first-order call-sequence structure (over both k-means tokens and the validated catalogue call types, site-controlled), and **context-specific production of communicative calls across more than one movement-defined behavioural context, with the individual held out** (DTAG H5: foraging/non-foraging and a three-way foraging/travelling/resting decode, with call-type × context selectivity and rate/loudness/echolocation controls), with the recording-site confound explicitly quantified. It does **not** claim translation, semantic understanding, referential call-level meaning, or any response to broadcast signals. The behavioural-context result is the *production* side of context-specificity (which call types are emitted in which context); it is **not** evidence that receivers interpret the calls differently by context, which is the perception side and is out of reach of an archival corpus. The call-type result is call-type discrimination (labels correlate with pod/matriline), not evidence of meaning. The sequence-structure result is a prerequisite for combinatorial coding, not evidence of meaning; demonstrating a measured response to a broadcast signal requires field playbacks this archival corpus cannot provide.
...
- "response proxy" rather than "playback response"
