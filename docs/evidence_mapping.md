...
| H6 (playback) | **The response criterion is met:** wild killer whales produce a measurable, selective behavioural response to broadcast **conspecific** calls — reply vocally to same-pod and stay silent to different-pod calls (6/6 vs 0/6 after pseudoreplication control, Fisher exact p = 0.002), often matching the played call type — and frozen AVES2 recovers the Kamchatka dialect call types that drive this (leave-one-out purity 0.439 vs a 0.05 label-shuffle null, p = 1e-3). Corroborated across independent broadcast-response datasets (heterospecific playback with matched controls [@selbmann2026aversive]; orca-call structure drives receiver heading change [@bowers2018]; natural vocal matching [@miller2004repertoires]). | Re-analysis of a published conspecific playback [@filatova2011playback] with the per-trial table recomputed (`run_playback_response_stats.py`); embedding separability on the public FEROP catalogue [@russianorca_catalogue] with a label-permutation null (`run_playback_response.py`); cross-dataset roll-up (`summarize_broadcast_response.py`). | Meets the **response criterion** (receivers act selectively on a broadcast endogenous signal, naive animals) — NOT referential meaning. The behavioural experiments are **prior published work** re-analysed here; the response tracks **dialect membership, not call content**; conspecific n is modest (robustness from cross-dataset convergence) [@filatova2011playback; @selbmann2026aversive; @bowers2018]. |
| Call-type context (catalogue) | The recovered catalogue call types specialize across two functionally distinct contexts: 72% (13/18) are single-context foraging- or socializing-specialists (foraging-specialists N4/N9; socializing-specialists the multi-pod two-voiced/identity calls). | Literature-grounded mapping of the validated Rung-1 call types to published field ethograms (context labels not from embeddings → non-circular); specialization index and disjointness Fisher test (`run_calltype_context_specialization.py`). | Production-side reading of "more than one behavioural context" at the named-unit level [@ford1989; @foote2008]; not a decode and not a claim of meaning; n = 18 types, disjointness test underpowered (p = 0.069). |
