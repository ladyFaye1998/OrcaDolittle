...
Beyond first-order ordering, Southern-Resident S-call sequences carry the structure the
comparative literature treats as a combinatorial prerequisite: against first-order Markov
surrogates the two-back call adds $0.645$ bits ($p\approx10^{-3}$; candidate phrase
S01$\to$S04$\to$S01, $z=7.95$), though Northern-Resident N-calls show none. At the named-unit
level the validated call types are documented across six functionally distinct behavioural
contexts \citep{ford1989,foote2008,riesch2008}. On the perception side, re-analysis of a
published conspecific playback experiment shows wild killer whales respond selectively to
broadcast calls---a vocal reply to every same-pod and no different-pod playback ($6/6$ vs
$0/6$, Fisher $p=0.002$, naive free-ranging animals)---and frozen AVES2 recovers the dialect
call types that drive it (leave-one-out purity $0.439$ vs a $0.05$ null)
\citep{filatova2011playback,russianorca_catalogue}. We release all code, a hash-frozen
artifact, and exact reproduction commands. The work is fully non-invasive and makes no claim of
semantic meaning; its single response result re-analyses prior published playbacks and tracks
dialect membership, not call content.
\end{abstract}
