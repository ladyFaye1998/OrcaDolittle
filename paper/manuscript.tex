...
trigram beats a bigram by $0.09$ bits/token, and the strongest over-represented three-call
...
bouting. At least one resident population therefore shows the stronger, beyond-first-order
...
\citep{selbmann2026aversive,bowers2018}. This meets the response criterion---a measurable,
...
\paragraph{Second-encoder robustness.} Repeating the FEROP dialect-space and site-controlled
...
$0.20$). This is representation stability, not a new semantic claim.
...
establishes a measurable, dialect-selective receiver response to broadcast conspecific calls,
...
but the labels are movement-only proxies and the on-tag call-type clusters are soft); and a
dialect-selective receiver response to broadcast conspecific calls (H6), by re-analysis of a
prior published playback (clean differential response, corroborated across independent
datasets, but modest conspecific $n$). \emph{Medium}: structure beyond first order in
Southern-Resident call sequences (H7; positive in SRKW, null in NRKW, small recording counts).
\emph{No claim (out of scope)}: referential call-level meaning, and that the receiver response
is governed by call content rather than dialect membership.
...
\section{Future work}
Every gap that public-archival re-analysis can close is closed here: validated call units,
