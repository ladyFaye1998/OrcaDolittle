...
($p=10^{-3}$): the encoder represents the dialect call-type space underlying the response contrast. The
...
inflate an acoustic decode or a response to an acoustic broadcast. \emph{Scope of inference}.
The most direct support is for site-controlled ecotype/call-type structure and cross-site
call-type transfer. The context result is production-side: calls are associated with more
than one movement-defined context with the individual held out, but the labels are proxies
and the on-tag clusters are soft. The response result is a re-analysis of a prior published
playback, corroborated by independent response datasets but bounded by modest conspecific
$n$. We make no claim of referential meaning, or that the receiver response is governed by
call content rather than dialect membership; that requires a controlled content-isolating
playback \citep{deecke2005,sayigh2025nsw}.
