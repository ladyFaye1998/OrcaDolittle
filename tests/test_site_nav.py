...
def test_navbar_uses_shared_assets_and_complete_link_set():
    ...
    for html in (INDEX, DICTIONARY):
        assert 'href="assets/site-nav.css?v=2"' in html
        assert 'src="assets/site-nav.js?v=2"' in html
...
def test_main_page_defers_nonvisible_field_kit_initialization():
    assert "requestIdleCallback(initializeLab" in INDEX
    assert "setMode('recordings',true)" in INDEX
    assert "renderTray(undefined,true)" in INDEX
